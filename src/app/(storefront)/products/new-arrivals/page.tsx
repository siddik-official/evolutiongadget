...
export const metadata: Metadata = {
  title: "New Arrivals",
  description:
    "Shop the latest new arrivals at Evolution Gadget. Cash on Delivery across Bangladesh.",
};
...
export default async function NewArrivalsPage({ searchParams }: Props) {
  ...
  try {
    ...
    // Build product query - fetch NEW products (sorted by created_at desc)
    let query = supabase
      .from("products")
      .select(
        ...
        <div className="flex-1">
          ...
          {products.length > 0 ? (
            ...
            <div className="text-center py-16 bg-muted/30 rounded-xl border border-dashed">
              <p className="text-muted-foreground">No new products found.</p>
            </div>
          )}
        </div>
      </div>
