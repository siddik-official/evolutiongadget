  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 bg-[var(--theme-dark)] text-[#E5E5E5] border-b border-white/10">
        <div className="container mx-auto pl-3 pr-1 sm:px-4">
          <div className="flex items-center justify-between h-14 sm:h-16 gap-1 sm:gap-4">
            ...
            {/* Desktop nav - Dynamic Navigation with dropdowns */}
            <div
              className={`transition-opacity ${searchExpanded ? "opacity-0 pointer-events-none" : "opacity-100"}`}
            >
              <DynamicNavigation />
            </div>
            ...
            <div className="flex items-center gap-0 sm:gap-2">
              ...
              <Button
                variant="ghost"
                size="icon"
                className="relative size-9 sm:size-10 text-[#E5E5E5] hover:text-white"
                onClick={() => setCartOpen(true)}
              >
                <ShoppingCart className="size-5" />
                {totalItems > 0 && (
                  <Badge className="absolute -top-0.5 -right-0.5 size-4 sm:size-5 flex items-center justify-center rounded-full p-0 text-[9px] sm:text-[10px]">
                    {totalItems}
                  </Badge>
                )}
              </Button>
              ...
              <Sheet open={mobileMenuOpen} onOpenChange={setMobileMenuOpen}>
                <SheetTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="lg:hidden size-9 sm:size-10 text-[#E5E5E5] hover:text-white"
                  >
